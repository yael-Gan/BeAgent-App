...
export default function About() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-blue-50 via-white to-blue-100">
      <Header />
<main className="flex-grow container mx-auto px-6 py-32 md:py-36 relative">
  {/* כותרת */}
  <motion.h1
    className="text-5xl md:text-6xl font-extrabold text-center mb-16 text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-teal-400 drop-shadow-lg relative max-w-5xl mx-auto leading-tight"
    variants={fadeUp}
    initial="hidden"
    animate="visible"
    transition={{ duration: 1 }}
  >
    BeAgent
    <motion.span
      className="absolute -top-8 left-1/4 text-yellow-400 text-2xl"
      animate={{ rotate: [0, 360] }}
      transition={{ repeat: Infinity, duration: 4 }}
    >✨</motion.span>
    <motion.span
      className="absolute -top-10 right-1/3 text-yellow-400 text-xl"
      animate={{ rotate: [0, -360] }}
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
    {[
      {
        title: "🌍 החזון שלנו",
        text: "להפוך את תכנון החופשה שלכם לחוויה פשוטה, מהנה ומלאת השראה — בלי שעות של חיפושים ובלי בלבול, אלא במסע נעים ומדויק אל היעד הבא שלכם.",
        delay: 0.3
      },
      {
        title: "✈️ מה אנחנו מציעים",
        text: "מערכת חכמה להתאמה אישית של חופשות, סינון קל ונוח, הצעות ממוקדות וטיפים ישירות ממומחים בתחום התיירות. BeAgent – כי מגיע לכם לתכנן חופשה בראש שקט.",
        delay: 0.5
      }
    ].map((card, idx) => (
      <motion.div
        key={idx}
        className="relative bg-gradient-to-r from-blue-400 via-teal-300 to-blue-200 p-12 md:p-16 min-h-[280px] rounded-[50%_20%_50%_20%] shadow-[0_25px_40px_rgba(0,0,0,0.15)] hover:shadow-[0_35px_60px_rgba(0,0,0,0.2)] transition-transform duration-500 hover:scale-105 overflow-hidden"
        variants={fadeUp}
        initial="hidden"
        animate="visible"
      ...
      >
        ...
        <p className="text-white text-lg leading-relaxed">{card.text}</p>
      </motion.div>
    ))}
  </div>
  ...
  >
    ...
    <button 
      onClick={handlePlanClick}
      className="bg-gradient-to-r from-blue-500 to-teal-400 text-white px-12 py-5 rounded-full shadow-xl hover:shadow-2xl hover:scale-105 transition-transform duration-500 font-semibold text-lg md:text-xl"
    >
      גלו חופשות
    </button>
  </motion.div>
