...
export default function SearchFlights() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await axios.get("https://api.aviationstack.com/v1/flights", {
        params: {
          access_key: process.env.REACT_APP_AVIATIONSTACK_KEY, // עדיף להשתמש ב-env var
          dep_iata: departure,
          arr_iata: arrival,
          flight_date: dateFrom,
          limit: 10,
      ...
      console.error("שגיאה בשליפת נתונים מה־API:", err);
      setError("שגיאה בשליפת נתונים מהשרת. נסי שוב מאוחר יותר.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 md:pt-32">
      ...
      {/* תיבת חיפוש */}
      <motion.div
        className="bg-white/90 backdrop-blur-lg rounded-3xl p-6 mb-12 flex flex-col md:flex-row gap-4 items-end shadow-xl"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8 }}
      >
        {/** שדות חיפוש **/}
        {[
          { label: "מוצא (IATA)", value: departure, setter: setDeparture, placeholder: "TLV", type: "text" },
          { label: "יעד (IATA)", value: arrival, setter: setArrival, placeholder: "JFK", type: "text" },
          { label: "תאריך יציאה", value: dateFrom, setter: setDateFrom, placeholder: "", type: "date" },
          { label: "תאריך חזרה", value: dateTo, setter: setDateTo, placeholder: "", type: "date" },
        ].map((field, idx) => (
          <div key={idx} className="flex-1 flex flex-col">
            <label className="text-gray-500 mb-1">{field.label}</label>
            <input
              type={field.type}
              value={field.value}
              onChange={(e) => field.setter(field.type === "text" ? e.target.value.toUpperCase() : e.target.value)}
              placeholder={field.placeholder}
              className="border-b-2 border-gray-300 focus:border-blue-500 outline-none py-2 transition-all rounded-md"
            />
          </div>
        ))}
        <button
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500">לא נמצאו טיסות</p>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {flights.map((flight, index) => (
            <motion.div
